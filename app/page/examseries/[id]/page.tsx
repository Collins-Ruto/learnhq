...
function Exams({ params: { id } }: { params: { id: string } }) {
  ...
  const name = convertToSlug(id);

  const [exams, setExams] = useState<(Exam & { student: Student & {stream: Stream}})[]>();
  const { data, isLoading, error  } = api.exam.getByName.useQuery(id);
...
//   const { data: slugExams } = api.exam.getByName.useQuery(name);
//   const { data: count } = api.exam.count.useQuery();
...
function convertToSlug(input: string): string {
  // Split the input string by dashes and spaces
  ...
  // Assemble the parts into the final slug
  const slug = `${year}-${form?.toLowerCase()}-${term.toLowerCase()}`;
